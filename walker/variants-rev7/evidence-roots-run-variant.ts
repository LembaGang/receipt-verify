...
import type { EvidenceEntry, EvidenceSet } from "../../tools/evidence-root.js";

// The only behavioural change against tools/evidence-roots-run-rev7.ts: the
// implementation under test is named on the command line rather than imported
// statically, so one harness runs against the baseline and against each of the
// three counterfactual builds. Everything below this block is that file verbatim,
// including `produced_by`, which names the harness program and is deliberately
// left unchanged: were it rewritten per run, the four outputs would differ in
// that one string and the comparison would be about the harness rather than the
// implementation. See walker/variants-rev7/README.md.
const {
  LEAF_PREFIX,
  NODE_PREFIX,
  computeRoot,
  ...
  rootsAgree,
  rootsDiffer,
} = (await import(pathToFileURL(resolve(process.argv[2]!)).href)) as typeof import(
  "../../tools/evidence-root.js"
);
