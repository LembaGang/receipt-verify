...
**Appended 2026-09-25 — -09 as posted, against the three -09 statements above**

Every line number in this block refers to `refs/draft-marques-asqav-compliance-receipts-09.txt` (sha256
`c455cdf8a402c46bff383088321d52d7aaebfef32090fd633efcdf02e7bee406`, 379,060 bytes, 7,840 lines) unless it
says -08. Each cited line was printed from the pinned bytes and read before this block was written.

**(i) On the N1 append (lines 192 to 249 of this file).** Two sentences of the author's 8 September answer quoted above do not describe -09 as posted on 21 September 2026 (sha256 c455cdf8a402c46bff383088321d52d7aaebfef32090fd633efcdf02e7bee406, pinned in refs/). -09 does not point at the upstream representation: Section 5.2.7 (lines 1094 to 1130) defines the Action descriptor itself, four members with scopeRequired sorted by UTF-16 code unit, and Appendix B.1 (lines 7486 to 7489) calls the Action-descriptor digest self-contained. A verifier holding the retained descriptor does recompute action_ref: Section 11.2 (lines 4569 to 4574) makes that a check when the selected evidence policy requires it. Still true: the receipt alone does not carry the descriptor (lines 1109 to 1114), and the seven values in this corpus are SHA-256 of zero bytes, which no four-member descriptor can produce, the smallest such descriptor under Section 5.2.7 being 64 bytes of JSON. The author said the first point himself on 24 September 2026. The fourteen declared_opaque entries built on the quoted sentence are withdrawn in walker/scopes.json in the commit that follows this append, on the same date, and action_ref is graded under a rule taken from Section 5.2.7 from the 6137cb95 corpus onward.

**(ii) On the M7 append (lines 392 to 407 of this file).** -09 does not say what the author's mail of 8 September described. It has no Section 10.5 on the verdict; the verdict is Section 11.5 and anchoring is Section 5.5. Issuers SHOULD obtain timestamp evidence (lines 1414 to 1415). An absent or empty anchors array means no anchor evidence was presented and is conformant (lines 1449 to 1450); null is malformed under this revision (lines 1450 to 1452). Whether absent evidence prevents full verification depends on the required axes of the selected profile and relying-party policy (lines 1452 to 1462), the required-axis set comes from that policy and not from a test fixture (lines 4632 to 4634), and a pending required anchor blocks full verification while an optional one stays pending on its own axis (lines 4719 to 4722). The part about reporting anchoring as its own axis holds, at Section 5.5 (lines 1436 to 1438 and 1461 to 1463) and Section 11.4 (lines 4617 to 4627). The author withdrew the rest on 24 September 2026. M7 remains true of -08 (lines 1052 and 1074 to 1075 of -08) and of the corpus at 05c1c49. It stays open until the corpus -09 pins, asqav-sdk 6137cb95 (-09 lines 7094 to 7097), is walked against -09, and its question changes: which policy each vector's expected outcome assumes, whether that policy requires the anchor axis, and how the null vectors are treated now that null is malformed and absent or empty is conformant. That walk is FINDINGS-rerun-2026-09-25.md.

**(iii) On lines 337 to 338 of this file ("which does not exist as bytes").** -09 now exists (posted 21 September 2026). Its Section 4 (lines 677 to 679) and Section 5.8.1 (lines 1700 to 1712) give envelope_hash one scope, the envelope minus anchors, and Section 5.8.1 adds the scope member (lines 1781 to 1802). The text half of M5 is resolved in -09. The section is 5.8 in -09, not 5.7.

**(iv) On lines 381 and 409 to 411 of this file (the M7 class "[], null or absent").** This census counts one class under -08 and two under -09: an empty array and an absent member both mean no anchor evidence was presented (Section 5.5, lines 1449 to 1450), while null is malformed unless an explicitly selected legacy format says otherwise (lines 1450 to 1452). The -09 walk reports them separately.

**(v) On lines 293 to 301 of this file (the v list relayed from 609c1b84).** The author wrote on 11 September 2026 that this list was accurate at 609c1b84 and no longer describes the tree. v is counted from the bytes at 6137cb95 in FINDINGS-rerun-2026-09-25.md, not from this list.
