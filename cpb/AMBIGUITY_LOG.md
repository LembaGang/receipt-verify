...
same session as this write (canon R37).
...
opened. No third-party implementation was consulted. Under the Lead's ratification of 2026-08-30,
`vectors/CANONICALIZATION_DECLARATION.md` and `vectors/README.md` were not opened, and
`vectors/subject-binding-diff/README.md` was not opened before this implementation was frozen and
committed. RFC 8785 itself was not fetched or read this session — see the bound in §0 below.
...
independent implementation of JCS. §4.1 step 1 delegates to [RFC8785], and the handoff required
reuse of the canonicalizer already vendored in this repository (`@headlessoracle/chirindo` 0.4.0,
`dist/vendor/recorder/index.js`) rather than adding a second JCS. So the JCS bytes come from code
...
**§8 typed digest references.** The handoff said to implement §8 only if §5 or §7.1 depends on it.
