...
Run 2026-08-30 under the Lead's authorisation of the same date. Implementation still frozen at
...
Readability is decided **mechanically**, by `carriesNormalizableMember` in `cpb/run-vectors.ts`, not
...
No row was unreadable, so the Lead's fallback — "if the vectors' own inputs make part 2 unreadable,
say so and keep part 1" — did not have to be used. Had any row carried a surviving null or empty
member, it would have been reported as a divergence attributable to the construction difference the
38 already measured, and to nothing else.
...
**That is byte-identical to the value our T2 tests already carried**, computed from the draft's
Appendix A worked example with no vector in sight. T2 labelled it a REGRESSION PIN and said plainly
...
- searched the whole tree at that commit, the digest `1009a072` appears in exactly one file —
  ...
  Appendix A;
- the only mention of `jcs-n/derived-id` anywhere at that commit is `cpb/PROVENANCE_T1.md:85`, T1's
...
`cpb/AMBIGUITY_LOG.md` §0: the JCS step is the vendored canonicalizer, not an independent
implementation of RFC 8785.

---

## COVERAGE — what T3b leaves unestablished
