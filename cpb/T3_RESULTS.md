...
| `jcs-n/derived-id` | 3 | `jcs-n` | declares the withdrawn construction; the ratification widened the observation to the 38 kats only |
| `jcs-n/assembled-preimage` | 2 | `jcs-n` | same |
| `domain-transforms/pass` + `/fail` | 2 | `jcs-n` | same |
| `multimodal/pass` | 1 | `jcs-n` | same |

**One correction to the enumeration in the ratification.** The ruling named the 31 vectors declaring
no top-level algorithm as "cpb-check grammar, typed-refs, registry, domain-transforms, multimodal,
profile-independence". Re-derived from the files at `e0ad1c7`: the 31 are `cpb-check` (13),
`typed-refs` (8), `registry` (4), `profile-independence` (2) — **and `subject-binding-diff` (4)**,
which is the PRIMARY set itself. `domain-transforms` (2) and `multimodal` (1) are not in that 31;
they declare `jcs-n` and belong to the 46. The ruling's intent — everything but the live set and the
38 kats is out of scope — is unaffected, and was applied.

**One flagged for the Lead, not run.** `jcs-n/derived-id/` holds `01-basic-derived-id`,
...
implementation covers and which PRIMARY never reaches. They declare `jcs-n`. Running them was not
authorized and was not done; whether a future cycle should is a decision for the Lead.
