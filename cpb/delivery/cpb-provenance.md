...
Written 2026-08-30, before any implementation code was written. This file records the inputs the
implementation was pinned to: the draft bytes, their repository commit, the vector inventory, and
the platform and runtime.

Every negative in this file names the scope it was drawn from. Provenance tags:
...
this machine, not a difference in their content.
...
`lib/` was **not entered, listed, opened or grepped**: written from the draft text alone.
...
### 3d. NOT READ — declared, not inferred
...
suite. The sources used were the -02 draft text and the KAT input/expected files; these three were
left unopened rather than assumed to sit inside that set. The draft's own §2
...
## 4. THE README SIGNPOST AND PR #72
...
Its base sha is exactly our clone commit, so `main` has not moved since the PR was opened.
The PR's diff was **not fetched** — only the file-list metadata above — so this file makes no
claim about the replacement wording.
...
The reason a renumbering was possible at all, from the draft's own §2 `Changes from -01`
(line 243, read from the filed text and not from any summary of it): the charter rescope removed the
...
**No mismatch. 4.1, 5 (with 5.1) and 7.1 are the correct three sections in -02, and the
implementation targets them.**
...
**Positive control on the instrument, one vector only.** `jcsBytes({b:"x",a:"y"})`
...
not a conformance result and forms no part of the vector runs reported separately.
...
records what changed afterwards, and narrows the claim.**

§2 states that `lib/` "was not entered, listed, opened or grepped". That held throughout the
writing of the implementation. It stopped being true, in one respect, **after** the
implementation was frozen and committed as `1886b405f31030c67a28e3c09fb09bc7446ffbe8`.

**What happened.** After the freeze, on opening
`vectors/subject-binding-diff/README.md`, that file named its harness as
`check_vectors.py`. No such file appeared in the inventory in §3, whose scope was `vectors/` only.
Rather than report an absence from a partial search, the filename was
...
**Why it cannot have affected the implementation.** The implementation was already committed at
...
`.github/check_vectors.py`, not under `vectors/`. The README's reference is accurate; the
inventory in §3 simply did not cover `.github/`. No absence claim was made or escalated on it.
