...
**CORRECTION 2026-09-18.** This heading, and every count stated between it and the claim manifest below (at `9d3cd19`: lines 220, 229, 247, 253, 270, 294 and 359), describes what the search can see and not what the corpus holds. Eighteen is the count of files that **pin a section 5 answer in their own file**; nineteen is the count of **pinned derivations** across those files. At least one section 5 input pins nothing at all: `profile-independence/pass/01-conforming-typed-ref.json`, its `profile_a` object, which declares `"algorithm": "jcs-n"` and the exclusion set `["record_id"]` over a payload whose `record_id` is JSON null, reduces to a 162-byte JCS pre-image and the identifier `799f0502971440d468f253fcdeee7a3c24f919e9b5454a8d245d93fe30d1f948` — a string that appears in no file at `e0ad1c7`. It was found by the corpus's author on 16 September 2026 and is reproduced here by the same code path that reproduces the same file's pinned `profile_b` value `0c837d01…` as a control. That file was already one of the eighteen, counted for `profile_b`, so the file count does not move; what moves is the count of inputs. **The section 5 inputs at `e0ad1c7` are therefore at least twenty.** Eighteen and nineteen are properties of the method — a search that keeps a candidate only where the answer is already written down in the same file cannot see an input that writes it down nowhere — and they are stated as such here rather than as a count of the corpus. **The cover letter of 4 September 2026, which stated "Eighteen files", is corrected in the same terms.** Every sentence below this block is left exactly as it was sent.
